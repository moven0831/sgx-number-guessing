...
contract MockDcapAttestation {
    ...
    function verifyAndAttestOnChain(bytes calldata) external view returns (bool success, bytes memory output) {
        // success
        success = true;

        string memory outputPath = string.concat(vm.projectRoot(), "/test/sample/output.bin");
